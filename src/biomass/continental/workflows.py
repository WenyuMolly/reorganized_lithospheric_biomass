"""Reusable helpers for R-based continental biomass estimation workflows.
...
def get_output_dir(workflow: str, base_dir: str | None = None) -> Path:
    ...
    base_dir : str, optional
        Output subdirectory name under `runs/continental/`. If omitted, use the
        active `BIOMASS_RUN_ID` if set, otherwise create a timestamp. Use
        "submitted" for reference outputs.
    ...
    workflow = validate_workflow(workflow)
    if base_dir is None:
        base_dir = make_run_id()
    return PROJECT_ROOT / "runs" / "continental" / base_dir / workflow


def make_run_id() -> str:
    """Return the active run id, creating a timestamp if none is set."""
    return os.environ.get("BIOMASS_RUN_ID") or datetime.now().strftime("%Y%m%d_%H%M%S")
...
def run_r_script(
    ...
    run_id : str, optional
        Output run id. If omitted, use `BIOMASS_RUN_ID` or create a timestamp.
    ...
    env = os.environ.copy()
    env["BIOMASS_RUN_ID"] = run_id

    warnings.warn(
        f"Running R script {script_name}. "
        f"This may take several minutes for bootstrap iterations. Output run: {run_id}",
        UserWarning
    )
...
def run_workflow(
    ...
    run_id : str, optional
        Shared output run id for all scripts in this workflow. If omitted, use
        `BIOMASS_RUN_ID` or create a timestamp.
    ...
    >>> from biomass.continental.workflows import run_workflow
    ...
    run_id = run_id or make_run_id()
    print(f"[INFO] Continental output directory: {get_output_dir(workflow, run_id)}")
    ...
    for script_name in scripts:
        ...
        try:
            result = run_r_script(script_name, workflow=workflow, run_id=run_id)
...
def load_results(
    ...
    >>> from biomass.continental.workflows import load_results
