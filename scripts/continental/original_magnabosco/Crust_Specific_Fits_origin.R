...
write.csv(newEstimate, file = "Origin_CSF_bootstrap_total_biomass_and_mse.csv", row.names = FALSE)
write.csv(gridValues, file = "Origin_CSF_bootstrap_grid_cell_biomass.csv", row.names = FALSE)
write.csv(parameters, file = "Origin_CSF_bootstrap_model_parameters.csv", row.names = FALSE)

# Extract R² columns and save
r2_columns = seq(3, ncol(parameters), by=3)
r2_matrix = parameters[, r2_columns]
write.csv(r2_matrix, file = "Origin_CSF_bootstrap_crusttype_rsquare.csv", row.names = FALSE)

# Save everything together
save(newEstimate, gridValues, parameters, r2_matrix, file = "Origin_CSF_full_bootstrap_results_with_rsquare.RData")
